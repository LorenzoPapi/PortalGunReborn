				...
				for (TileEntity tileEntity : render.compiledChunk.get().getTileEntities()) {
					for (BlockPos position : pairStruct.positions) {
						if (tileEntity.getPos().equals(position)) {
							...
							/* start render chunk */
							BlockPos relPos = render.getPosition().subtract(tileEntity.getPos());
							matrixStackIn.push();
							...
							for (int x = 0; x < 16; x++) {
								for (int y = 0; y < 16; y++) {
									for (int z = 0; z < 16; z++) {
										if (xOff < 0) {
											if (render.getPosition().getX() + (x + xOff) >= tileEntity.getPos().getX())
												continue;
										} else if (xOff > 0) {
											if (render.getPosition().getX() + (x + xOff) <= tileEntity.getPos().getX())
												continue;
										} else if (zOff < 0) {
											if (render.getPosition().getZ() + (z - zOff) <= tileEntity.getPos().getZ())
												continue;
										} else if (zOff > 0) {
											if (render.getPosition().getZ() + (z - zOff) >= tileEntity.getPos().getZ())
												continue;
...
//				}
		...
		WorldRenderer.drawBoundingBox(
				...
				tileEntityIn.getRenderBoundingBox(),
				0.5f, 0, 1, 1
		);
	}
