package com.github.lorenzopapi.pgr.entity;
...
public class PortalProjectileEntity extends Entity {
	...
	private static final DataParameter<Integer> COLOUR = EntityDataManager.createKey(PortalProjectileEntity.class, DataSerializers.VARINT);
	...
	private static final DataParameter<Integer> DISTANCE = EntityDataManager.createKey(PortalProjectileEntity.class, DataSerializers.VARINT);
	...
	public PortalProjectileEntity(World worldIn, Entity shooter, int width, int height, PortalStructure info, int distance) {
		this(worldIn);
		...
		this.pWidth = width;
		this.pHeight = height;
		this.structure = info.setWidthAndHeight(width, height);
		getDataManager().set(COLOUR, this.structure.colour);
		getDataManager().set(DISTANCE, distance);
		Vector3d location = EntityHelper.getEntityPositionEyes(shooter, 1.0F);
		...
		double velo = 4.98D + Math.random() * 0.02D;
		setMotion(look.x * velo, look.y * velo, look.z * velo);
				...
				if (!this.world.isRemote) {
					...
					if (PortalGunHelper.spawnPortal(this.world, pos, brtr.getFace(), look, this.structure, this.pWidth, this.pHeight)) {
						if (this.shooter != null) {
							...
							EntityHelper.playSoundAtEntity(this.shooter, this.structure.isTypeA ? PGRSoundEvents.p_portal_open_blue : PGRSoundEvents.p_portal_open_red, this.shooter.getSoundCategory(), 0.2F, 1.0F + (this.shooter.getRNG().nextFloat() - this.shooter.getRNG().nextFloat()) * 0.1F);
						}
					} else {
						if (this.shooter != null) {
							EntityHelper.playSoundAtEntity(this.shooter, PGRSoundEvents.pg_portal_invalid_surface_swt, this.shooter.getSoundCategory(), 0.2F, 1.0F + (this.shooter.getRNG().nextFloat() - this.shooter.getRNG().nextFloat()) * 0.1F);
						}
						EntityHelper.playSoundAtEntity(this, PGRSoundEvents.p_portal_invalid_surface, SoundCategory.BLOCKS, 0.4F, 1.0F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F);
					}
				}
				setDead();
				return;
	...
	protected void setDead() {
		super.setDead();
		if (!this.world.isRemote) {
			PGRChunkHandler.removeTicket(this);
		}
	}
